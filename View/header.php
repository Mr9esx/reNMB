<?php
	if(defined('IN_SYS')) {

	}else{
		echo "非法访问！";
		exit();
	}
echo "
<!DOCTYPE html>
	<head>
		<link rel='stylesheet' href='".WEBROOTURL."/css/font-awesome.min.css'>
		<link rel='stylesheet' href='".WEBROOTURL."/css/bootstrap.min.css'>
		<link rel='stylesheet' href='".WEBROOTURL."/css/style.css'>
		<link href='".WEBROOTURL."/css/jquery-accordion-menu.css' rel='stylesheet' type='text/css' />
		
		<script type='text/jscript' src='".WEBROOTURL."/js/jquery.min.js'></script>
		<script type='text/jscript' src='".WEBROOTURL."/js/bootstrap.min.js'></script>
		<script type='text/javascript' src='".WEBROOTURL."/js/func.js'></script>
		<script type='text/javascript' src='".WEBROOTURL."/js/jquery-accordion-menu.js'></script>
		<script type='text/javascript' src='".WEBROOTURL."/js/jquery.form.min.js'></script>

		<link href='favicon.ico' mce_href='favicon.ico' rel='icon' type='image/x-icon' /> 
		<meta http-equiv='Content-Type' content='text/html; charset=utf-8' />
		<title>
";
echo getWebName();
echo "
</title>
	</head>
";
?>
